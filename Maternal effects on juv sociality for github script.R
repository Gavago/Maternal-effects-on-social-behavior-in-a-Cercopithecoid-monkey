...
mdfj_ajp <- read.csv(text=getURL("https://raw.githubusercontent.com/Gavago/Tidyverse-tutorial-CU/master/rainfall.csv"),stringsAsFactors = F, header=T)
mjdf_ajp <- read.csv("/Users/nic/Documents/NYCEP/2017/writing/5. mum-juv paper/AJP paper/ajp for github/maternal effects blue monkeys social.csv", stringsAsFactors = F) %>%
  mutate(mum = as.factor(mum))  
dim(mjdf_ajp) #41 21


# Data for mum juvenile association over time (part 4)
mjdf_year_ajp <- read.csv("/Users/nic/Documents/NYCEP/2017/writing/5. mum-juv paper/AJP paper/ajp for github/maternal effects blue monkeys social _ groom and near data by year.csv", stringsAsFactors = F) %>%
  mutate(mum = as.factor(mum)) %>%
...
fy.mj.gg.prs<-gamlss(j.p.gg ~ z.(fy.mj.gg) + sex + z.(age)  + z.(m.rank.sp) +  z.(fy.mj.gg)*sex + random(mum) , family="BE",data=mjdf_ajp)
...
fy.rc.prs<-gamlss(p.rc.j ~ z.(fy.p.rc.af) + sex + z.(age)  + z.(m.rank.sp) +  z.(fy.p.rc.af)*sex+ random(mum) , family="BE",data=mjdf_ajp)
  # by mother's groomin with juv as infant
fy.mj.rc.prs<-gamlss(j.p.rc ~ z.(fy.mj.rc) + sex + z.(age)  + z.(m.rank.sp) +  z.(fy.mj.rc)*sex + random(mum) , family="BE",data=mjdf_ajp)
...
fy.pl.prs<-gamlss(p.pl.j ~ z.(fy.p.gg.af) + sex + z.(age)  + z.(m.rank.sp) +  z.(fy.p.gg.af)*sex+ random(mum) , family="BEZI",data=mjdf_ajp, method=RS(50))
  # by mother's groomin with juv as infant
fy.mj.pl.prs<-gamlss(j.p.pl ~ z.(fy.mj.gg) + sex + z.(age)  + z.(m.rank.sp) +  z.(fy.mj.gg)*sex + random(mum) , family="BE",data=mjdf_ajp)
...
# (3) Mother juvenile association over time
