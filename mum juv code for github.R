...
########
# Models of influence in first year
load("mjdf for GAMs.Rdata", verbose = T)
dim(mjdf) #41 75
...
# Figure 2 ------
plot1 <- mjdf %>%
  ggplot(.) + geom_point(aes(x = 100*mj.gg_avg_lifetime, y = 100*j.p.gg, color = sex, shape = sex), show.legend = F) +
  geom_smooth(aes(x = 100*mj.gg_avg_lifetime, y = 100*j.p.gg, group = sex, color = sex, linetype = sex), method = "lm", show.legend = F) +
  scale_colour_manual(values=cbPalette) +
  labs(y = "", x = "% Time mom groom w juvenile over juv lifetime") +
  ggtitle("a) % Time juvenile groom with any partner") +
  theme(plot.title = element_text(hjust = 0.5, size = 12))

plot2 <- mjdf %>%
  ggplot(.) + geom_point(aes(x = 100*mj.gg_avg_lifetime, y = 100*p.gg.j, color = sex, shape = sex)) +
  geom_smooth(aes(x = 100*mj.gg_avg_lifetime, y = 100*p.gg.j, group = sex, color = sex, linetype = sex), method = "lm") +
  scale_colour_manual(values=cbPalette) +
  labs(y = "", x = "% Time mom groom w juvenile over juv lifetime") +
...
plot3 <- mjdf %>%
  ggplot(.) + geom_point(aes(x = 100*p.gg.af_avg_lifetime, y = 100*p.pl.j, color = sex, shape = sex), show.legend = F) +
  geom_smooth(aes(x = 100*p.gg.af_avg_lifetime, y = 100*p.pl.j, group = sex, color = sex, linetype = sex), method = "lm", show.legend = F) +
  scale_colour_manual(values=cbPalette) +
  labs(y = "", x = "% Time mom groom w peers over juv lifetime") +
  ggtitle("c) % Time juvenile play with peers") +
  theme(plot.title = element_text(hjust = 0.5, size = 12))


# Figure 3 ------
dat %>% # dat loaded under "mother juv association over time"
  mutate(`a) grooming with groupmates` = m.p.gg_not_juv, `b) near groupmates` = m.p.rc_not_juv,
         `c) grooming offspring` = mj.gm, `d) groomed by offspring` = mj.gg - mj.gm,
         `e) near offspring` =  mj.rc) %>%
  gather(key = aff, value = percent_time, `a) grooming with groupmates`, `b) near groupmates`, `c) grooming offspring`, `d) groomed by offspring`, `e) near offspring`) %>%
  ggplot(.) +
  ...
  facet_wrap(facets = vars(aff), nrow = 2, scales = "free_y")